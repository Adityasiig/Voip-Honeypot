# HoneyPot — VoIP fraud honeypot and compliance panel
...
2. Create the panel user and directories:
   ...
   ```
3. Copy `panel/*.py` to `/opt/voip/`, `examples/azure.env.example` to `/opt/voip/.azure.env`
...
* Source-IP admission control (Settings → Platform defaults) rejects calls from IPs not
